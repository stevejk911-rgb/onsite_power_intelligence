...
async function nearbyInfra(lat, lon) {
  ...
  // out geom -> we get full way geometry and measure distance to the NEAREST vertex
  // (not the centroid), matching the main app's distances.
    ...
    try {
      ...
      for (const el of (j.elements || [])) {
        ...
        if (Array.isArray(el.geometry)) {
          for (const g of el.geometry) { const dd = haversineKm(lat, lon, g.lat, g.lon); if (dd < dmin) dmin = dd; }
        } else {
          const c = el.center || (el.lat != null ? { lat: el.lat, lon: el.lon } : null);
          if (c) dmin = haversineKm(lat, lon, c.lat, c.lon);
        }
  ...
  return { gas_km: null, line_km: null, substation_km: null, source: "unavailable" };
}
...
// Proximity-aware grid verdict. Regional queue year alone over-flags KILL on
// parcels that already sit next to grid infrastructure (e.g. operating sites).
// Existing transmission/substation nearby => capacity may already be available.
function gridVerdict(g, infra) {
  const gy = g.energization_year_est, fast = (gy - NOW) <= 3;
  const tx = infra.line_km, sub = infra.substation_km;
  const near = (tx != null && tx <= 1.5) || (sub != null && sub <= 2);
  const known = (tx != null || sub != null);
  const far = (tx == null || tx > 5) && (sub == null || sub > 5);
  const nums = [tx, sub].filter(function (v) { return v != null; }).sort(function (a, b) { return a - b; });
  const minkm = nums.length ? nums[0] : null;
  ...
  if (near) return { verdict: "CHECK", reason: `Transmission/substation is adjacent (~${minkm} km), so existing or upgradeable grid capacity may be available here — but new large-load interconnection in ${g.label} runs ~${gy}.` + tail };
  if (known && far) return { verdict: "KILL", reason: `No grid infrastructure within several km, and new large-load interconnection in ${g.label} runs ~${gy}, so the grid path is unlikely within a typical build window.` + tail };
...
async function buildVerdict(lat, lon) {
  ...
  // Overall feasibility = the headline (matches the main app: ADVANCE if any path
  // delivers in window, even when grid alone is slow).
  let overall;
  if (anyAdvance) {
    overall = { verdict: "ADVANCE",
      reason: `At least one on-site path looks viable here (fastest: ~${gas.fastest_months_est || 18} mo, gas + BESS). The grid alone is slower (~${grid.energization_year_est}). Worth advancing to a feasibility study.` };
  ...
  return {
    ...
    fastest_path, any_path_advance: anyAdvance,
